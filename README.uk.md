...
| `Дочікуватись повного показу тексту вулиці` | Увімкнено | Ще не працює; увімкнення не змінює оновлення тексту |
...
Вікно експорту показує поточний етап і файл, а після пошуку — фактичну кількість файлів та скопійований обсяг. Збирання може тривати довго, а архів — перевищувати 1 ГБ. Його можна скасувати або перейти в інший застосунок і повернутися, поки експорт триває у фоні. Кінцевий розмір архіву з’являється лише після перевірки ZIP. Ліміт надсилання розробнику — 20 MiB; більші архіви залишаються доступними через `Інший застосунок`, без поділу чи обрізання.
...
Logcat безперервно записує в один файл, доки ви його не зупините. Окремого ліміту розміру запису немає, тому тривалі записи займають більше місця; звичайні налаштування зберігання журналів продовжують діяти.
