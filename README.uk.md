...
`Штатний` передає підтримувані ліміти (5–130 км/год із кроком 5) у штатне поле та перевіряє результат через ADAS. Якщо протягом 3 секунд підтвердження немає, використовується вибраний запасний режим; повторні спроби починаються не частіше ніж раз на 10 секунд. Збіг потрібного значення прибирає запасний вивід. Непідтримувані ліміти одразу використовують запасний режим. Зникнення навігаційного ліміту не очищує штатний знак. BYD HUD не очищує штатний знак, зокрема після завершення навігації; старі налаштування очищення не мають дії. Автомобіль може застосувати вже переданий ліміт пізніше, навіть після переходу на запасний режим або завершення маршруту; зупинка нашого виводу не скасовує прийнятий автомобілем запис.
