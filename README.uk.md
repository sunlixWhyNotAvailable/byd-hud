...
Примітка вгорі містить підтримувані збірки навігаторів. Дія `Скачати` отримує запропоновану збірку з релізу проєкту, показує прогрес, перевіряє цілісність і сумісність файла, а потім відкриває системний інсталятор Android. Сумісна встановлена версія оновлюється штатно; видалення пропонується лише після явного підтвердження, якщо Android не може безпечно оновити встановлений застосунок.
...
Для зменшення APK 68 ілюстрацій довідки стиснуті у WebP (якість 60) з початковою роздільністю 2172×724. Позиції елементів і мовні варіанти збережені; стиснення не впливає на навігаційні картинки, які надсилаються на HUD.
...
Навчання та перехоплення кнопки потребують увімкненої служби спеціальних можливостей. Призначена кнопка більше не виконує штатну дію, навіть якщо жест не має відповідного профілю, вибраний застосунок закритий або перенесення неможливе. Щоб повернути штатну функцію кнопки, видаліть всі її профілі.

Одиночне натискання спрацьовує після відпускання. Якщо цій самій кнопці призначено подвійне натискання, одиночна дія трохи чекає, щоб розрізнити два жести. Подвійне спрацьовує після другого короткого натискання, а для утримання потрібно затиснути кнопку. Підтримувані кнопки та жести залежать від прошивки автомобіля; нові режими натискання ще потребують перевірки на авто.
...
Коли автоматична перевірка оновлень увімкнена, BYD HUD перевіряє їх невдовзі після запуску, зокрема дозволеного фонового запуску. Доступні оновлення показуються, коли ви відкриваєте застосунок. Відхилене оновлення може бути запропоноване пізніше. За потреби можна перевірити оновлення вручну, зокрема після невдалої перевірки.
...
Експорт використовує компактну картку в кутку, не блокуючи інші вкладки. Вона показує етап, кількість та обсяг уже знайдених файлів, а після пошуку — фактичний прогрес копіювання; секунди оновлюються незалежно від прогресу файлів. Повні шляхи та пояснення доступні через `Деталі`. Збирання може тривати довго, а архів — перевищувати 1 ГБ. Підготовку можна зупинити або перемикати вкладки й застосунки, поки вона триває. Кінцевий розмір архіву з’являється лише після перевірки ZIP. Ліміт надсилання розробнику — 20 MiB; більші архіви залишаються доступними через `Інший застосунок`, без поділу чи обрізання.
...
Експорт конфігурації та надсилання логів за вибрані дні зберігають одну картку протягом підготовки, надсилання й показу результату. Картки розміщуються разом із картками патчення Waze і Google Maps, не перекриваючи їх. Під час надсилання `Закрити` приховує картку, але не скасовує завантаження й не дозволяє почати інший конфліктний експорт. Прогрес не відкриває приховану картку повторно; результат зберігається в діагностиці. Підготовку й далі можна справді скасувати, а згода запитується до створення чи надсилання архіву.
