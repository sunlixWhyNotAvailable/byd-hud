...
Збережений Waze lifecycle Store є єдиним джерелом рішення про повторне відкриття direct-маршруту після terminal fence. Він утримує прийнятий неактивний перехід `NEW_DEST`, `NEW_ROUTE_RECEIVED` або `NEW_ROUTE_REQUESTED` до надходження відповідної активної події, тоді як застарілі події, події з нижчим generation і replay snapshots того самого generation не можуть повторно відкрити маршрут. Перемикач HUD, розміщення на панелі приладів і callback-session fences каналів Cluster/Surface залишаються незалежними від цього lifecycle-рішення.
...
Вкладка `Ручний` є діагностичним інструментом. Вона може надсилати окремі значення тексту, відстані, смуг, PNG і штатної стрілки без запуску маршруту. Кожна коректна зміна оновлює канонічний знімок; швидкі зміни об'єднуються так, щоб у канали RoadInfo HUD і TBT панелі приладів надсилався останній стан. Це корисно для перевірки відповідності символів автомобіля, але не повинно використовуватися як джерело навігації.
...
Якщо наявний RSA-ключ ADB авторизований, BYD HUD одразу разом із постійним runtime запускає приватну shell-UID обгортку Instrument/TBT. Вона не очікує початку маршруту й не додає окремого індикатора в інтерфейсі. Без авторизованого ADB/RSA direct-вивід RoadInfo SOME/IP на HUD залишається доступним, але сумісність картки TBT через shell-UID Instrument недоступна. Протокол `30011`, штатний AMap-адаптер і публікація RoadInfo залишаються незалежними площинами.

Instrument proxy protocol v3 передає перевірені масиви напрямків і рекомендацій смуг разом із наявною підказкою маневру. Коли `Вивід смуг` увімкнено і direct-джерело надає смуги, BYD HUD записує однаковий нормалізований стан через BYD Setting fields і прямі Instrument/FID lane fields; порожня або термінальна підказка очищає ці поля. Помилка запису смуг не скасовує успішний маневр і дозволяє наступному guidance frame повторити об'єднаний payload. Публікація смуг через RoadInfo залишається незалежною та не потребує ADB.
...
| Картка TBT через Instrument/FID | Недоступна на перевіреній прошивці | Доступна через startup shell-обгортку |
