...
Custom surface діє лише в межах маршруту. Зміна перемикача під час активного маршруту не перемикає сесію наживо. Якщо surface не готовий протягом п'яти секунд, BYD HUD повертається до стандартної direct-сесії Waze і не повторює спробу до наступного маршруту. Перемикач попереджень Waze керує лише виводом попереджень на HUD; попередження, які Waze передає в custom surface, залишаються видимими там.
...
Прямий канал Google Maps надає повний набір показників. Стандартна cluster-сесія Waze не надає показники поїздки; активна custom-surface main-сесія може надати їх, якщо Waze включає ці значення.
...
- Стандартна cluster direct-сесія Waze не надає час прибуття, залишок часу або залишок дистанції; ці значення залежать від того, чи передає їх Waze у custom-surface main-сесії.
...
- Custom surface Waze є опційним beta-шляхом. Він потребує робочого direct-каналу Waze і запускається лише після повідомлення Waze про активну навігацію; якщо surface не готовий, через п'ять секунд відновлюється звичайна direct-сесія.
