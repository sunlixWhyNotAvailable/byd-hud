...
<img align="right" src="app/src/main/res/drawable-nodpi/hud_apk_icon.webp" alt="Іконка BYD HUD" width="112">
...
- **Версія:** v3.2.6 (тестовий кандидат; релізна ціль v3.3.0)
...
Для зменшення APK 83 ілюстрації довідки стиснуті у WebP (якість 60) з початковою роздільністю 2172×724. Іконки застосунку та верхньої панелі також використовують WebP із якістю 60, початковими розмірами та прозорістю. Позиції елементів і мовні варіанти збережені; стиснення не впливає на навігаційні картинки, які надсилаються на HUD.
...
| Налаштування | За замовчуванням | Поведінка |
| --- | --- | --- |
| `Режим виводу обмеження швидкості` | Вимкнено | Обирає `Вимкнено`, `Штатний`, `У полі з маневром`, `У полі зі смугами`, `У вільному полі` або `Композитний` |
| `Очищувати штатне поле обмеження швидкості` | Ніколи | Поза штатним режимом очищує поле лише під час активного HUD-виводу: ніколи, коли немає навігаційного ліміту, або завжди |
| `Запасний режим виводу обмеження швидкості` | Вимкнено | У штатному режимі обирає bitmap-вивід, якщо штатне поле недоступне або не підтвердило потрібний ліміт |
...
`Штатний` передає підтримувані ліміти (5–130 км/год із кроком 5) у штатне поле та перевіряє результат через ADAS. Якщо протягом 3 секунд підтвердження немає, використовується вибраний запасний режим; повторні спроби починаються не частіше ніж раз на 10 секунд. Збіг потрібного значення прибирає запасний вивід. Непідтримувані ліміти одразу використовують запасний режим. Зникнення навігаційного ліміту не очищує штатний знак. Очищення — окрема добровільна опція, доступна також при вимкненому виводі ліміту, але тільки під час активного навігаційного виводу на HUD. Штатний режим її ігнорує. Поведінку цього тестового кандидата ще потрібно підтвердити на автомобілі.

Підказка штатного режиму ілюструє 115 у штатному полі («Вимкнено» — 40); вибір у підказці не змінює налаштування та нічого не передає на HUD. Запасний режим доступний лише для штатного, а подальші bitmap-налаштування залежать від вибраного запасного режиму. Неактивні налаштування зберігають значення.
...
Закриття модалки оновлення не зупиняє активне завантаження. Повторна перевірка показує ту саму операцію та її поточний прогрес. Якщо скасувати системне встановлення, його можна повторити з підготовленого APK; доступну згодом новішу версію також можна завантажити. Після перезапуску процесу незавершене завантаження потрібно повторити вручну, а підготовлені файли для інсталятора зберігаються.
...
Для базового й розширеного виводу `Зупинити`, вихід із вкладки, згортання BYD HUD або перемикання режимів завершує перевірку та очищає її дані. Вивід із навігатора відновлюється, якщо його маршрут досі активний і має право на вивід. Повторний запуск зберігає вибраний приклад. Тестові значення не змінюють налаштування Waze чи Google Maps. Авторизований ADB вмикає додатковий автомобільний інтерфейс, потрібний на сумісних авто для штатних смуг та деяких інших полів; недоступність одного виходу не заважає перевірці інших. TBT-картка використовує власне розташування маневру й тексту, а не копію смуг або bitmap-зображення з HUD.

### Тест у Шанхаї

`Тест у Шанхаї` має власні кнопки Почати/Завершити. За наявності авторизованого локального ADB запуск встановлює штучну GPS-позицію в Шанхаї. Протягом початкових 15 секунд відкрийте штатний навігатор і запустіть навігацію з виводом на HUD. Збережений маршрут триває 4 хвилини 55 секунд разом із початковим очікуванням. Він продовжується при зміні вкладки чи переході до навігатора, не змінюючи налаштування автозапуску. На час тесту BYD HUD призупиняє власний вивід HUD/TBT, продовжуючи отримувати навігаційні дані.

Logcat записується обов'язково. Раніше початий запис продовжується під час і після тесту; інакше тест запускає власний запис та завершує його після очищення GPS і ще 5 секунд запису. Вибір режиму перевірки HUD і кнопка Logcat заблоковані до завершення. Разом із логами зберігаються доступні SOME/IP, ADAS, захоплення пакетів і знімки стану; недоступні канали й втрати позначаються. Завершення маршруту не підтверджує, що саме з'явилося на фізичному HUD.

`Скинути штучні координати` прибирає поточну підміну GPS, зокрема створену іншим застосунком, і завершує активний тест у Шанхаї. Автоматичне відновлення при відкритті/увімкненні BYD HUD очищає лише власну перервану сесію. Помилка очищення залишається видимою для повторної спроби. Після аварійного закриття відновлення потребує наступного відкриття/увімкнення застосунку; миттєва реальна позиція чи повернення мапи не гарантуються. Інший застосунок може почати нову підміну після ручного скидання.
