...
- надає `Поділитись конфігурацією` для меншого діагностичного архіву з даними про пристрій, дозволи, мережу, патчера і налаштування навігаційного виводу.

`Поділитись логами` і сортування залишаються вгорі секції `Логи`; її обмежений за висотою список днів прокручується окремо, тому нижні кнопки залишаються доступними. Внизу секції розташовані `Почати Logcat` / `Зупинити Logcat`, `Поділитись конфігурацією` та `Видалити вибране`. Доступні шляхи показано нижче в секції `Тека журналу навігації`. Надсилання конфігурації не потребує вибраних днів.
...
Для проблеми з дозволами, пристроєм, патчером або запуском скористайтеся `Сховище та логи -> Логи -> Поділитись конфігурацією`.
