...
| --- | --- | --- |
| `Режим виводу ЕТА/часу/дистанції` | Вимкнений | Обирає `Вимкнений`, `До зупинки` або `Весь маршрут`; Waze завжди надає лише значення до наступної зупинки |
...
Прямий канал Google Maps надає повний набір показників. Патчена в межах проєкту Waze `5.20.0.1` надає у cluster-сесії ETA та залишок дистанції до наступної точки призначення. Залишок часу й окремі показники всього маршруту ця сесія наразі не надає.

### Обмеження швидкості

| Налаштування | За замовчуванням | Поведінка |
| --- | --- | --- |
| `Режим виводу обмеження швидкості` | Вимкнено | Розміщує знак поточного обмеження з direct-каналу в полі маневру, полі смуг або першому вільному полі |
...
| `Час показу при накладанні` | 5 секунд | Установлює час тимчасової заміни від 1 до 10 секунд |

Попередження займає поле маневру з таким самим пріоритетом, як маршрутний маневр. Знак у справді вільному полі залишається до зміни або очищення direct-джерелом; таймер діє лише при заміні зайнятого поля маневру, попередження або смуг. Функція потребує актуального direct-профілю Google Maps або Waze, патченого в межах проєкту.
...
Шар прямого каналу обов'язковий. Опційні шари, наприклад навігаційний аудіоканал Google Maps або стабільна сесія і підтримка попереджень Waze, можуть бути недоступні, не заважаючи роботі прямого каналу. Шар стабільної сесії Waze охоплює життєвий цикл маршруту, пряме отримання обмеження швидкості та показники cluster-маршруту. Alert-hook Waze наразі обмежений структурно сумісною версією Waze 5.20.0.1.
...
- Патчена Waze `5.20.0.1` у cluster direct-режимі надає ETA та залишок дистанції лише до наступної точки призначення; залишок часу й показники всього маршруту залежать від іншої сесії Waze, яка їх передає.
