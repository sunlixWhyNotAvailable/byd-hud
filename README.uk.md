...
Перемикачі мови й теми в заголовку мають плавну рухому підсвітку. Вибір застосовується одразу, без очікування завершення анімації тривалістю 140 мс; збереження налаштувань і розміри елементів не змінені.

Кнопки, рядки з перемикачами, вибір днів логів і елементи перевірки HUD також запускають дії без штучної паузи перед виконанням. Ефекти натискання й анімації руху працюють незалежно від запуску дії; таймери розпізнавання жестів та очікування результатів операцій не змінені.
...
Заокруглена кнопка `?` біля базових налаштувань, ЕТА, обмежень швидкості та Waze відкриває схематичний передпоказ. Його локальні елементи керування змінюють лише картинку, а не збережені налаштування чи HUD. Приклади відповідають українській/англійській мові застосунку й вибраному формату вулиці; передпоказ кольору змінює лише відповідний текст. Експериментальні поля ЕТА/Waze використовують окремі області HUD. Налаштування очікування повного тексту зарезервоване й поки не впливає на вивід. Наведені нижче скріншоти зроблені до цього перенесення UI.
...
| `Дочікуватись повного показу тексту вулиці` | Увімкнено | Зарезервоване налаштування; наразі лише зберігається, без затримки оновлень тексту |
...
Три перемикачі значень неактивні в режимі `Вимкнений`, але їхні збережені значення не скидаються. Формат, очікування й кольори залишаються видимими, коли недоступні: формат потребує поля Вулиці, очікування додатково потребує режиму Приставити, а кожен колір — експериментального поля та ввімкненого відповідного показника.
...
Кнопки режимів потребують авторизованого ADB. Вони змінюють відображення без перенесення, запуску чи повернення навігатора. Якщо застосування профілю вимкнено або проєкції BYD HUD немає, надсилається лише запит режиму. Вибір профілю через віджет не змінює режим, збережений для наступної дії `На приборку`. Віджет не позначає поточний режим, оскільки стан прошивки неможливо надійно визначити.
...
Відкрийте `Налаштування → Налаштування перенесення на приборку` та натисніть `+ Створити профіль`. Визначте кнопку, виберіть `Одиночне`, `Утримання` або `Подвійне`, встановлений застосунок і `Поточний профіль`, `Тільки частковий` або `Тільки повний`. Поточний використовує збережений режим приборки в момент виконання жесту. Збереження доступне, лише якщо вибрані кнопка й застосунок та немає іншого профілю з цією фізичною кнопкою і типом натискання. Відомі короткий/довгий коди вважаються однією кнопкою.

Навчання та редагування залишаються чернеткою до збереження. Скасування відкидає їх. Карандаш відкриває редагування; видалення зі списку або редактора завжди потребує підтвердження. «Ні» залишає чернетку відкритою; «Так» видаляє початковий профіль. Профілі зберігаються між перезапусками; попереднє налаштоване призначення мігрує автоматично, а штатний довгий код стає «Утриманням».

Вибраний застосунок уже повинен бути запущений, а перенесення на приборку потребує авторизованого ADB. Кожне нове натискання перевіряє його поточне вікно й дисплей: з основного екрана застосунок переноситься на приборку з вибраним профілем, а з приборки повертається на основний екран. Якщо запущене вікно неможливо підтвердити, нічого не переноситься й не запускається. Повторні натискання під час перенесення ігноруються й не додаються в чергу. Це працює й після перенесення із вкладки «Застосунки». Невдале перенесення супроводжується повідомленням.

Навчання та перехоплення кнопки потребують увімкненої служби спеціальних можливостей. BYD HUD поглинає весь отриманий потік DOWN/repeat/UP призначеної фізичної кнопки, навіть якщо жест не відповідає жодному профілю, застосунок закритий, перенесення вже триває або перевірка завершується помилкою. Штатна дія не відтворюється. Щоб звільнити призначення кнопки, видаліть усі її профілі.

Одиночне натискання спрацьовує після відпускання; очікування системного інтервалу подвійного натискання додається лише за наявності профілю «Подвійне». Подвійне завершується після другого короткого відпускання. Відомий штатний довгий код одразу визначає «Утримання»; для кнопок без такого коду, зокрема Камера 294, використовується системний таймаут утримання. Зміна профілів або вимкнення runtime скасовує очікувані жести. Обладнання та прошивка визначають, які події отримує Accessibility; автомобільна перевірка нових режимів натискання ще попереду.
...
Коли автоматична перевірка оновлень увімкнена, перша перевірка починається приблизно через 30 секунд після відкриття BYD HUD або дозволеного фонового запуску. Доступне оновлення показується, коли застосунок на передньому плані. Після закриття вікно не з'являється повторно, доки процес застосунку працює; через годину наступне відкриття застосунку або запуск runtime може дозволити нову перевірку. Ручна перевірка доступна одразу, зокрема після помилки. `Вимкнути` або повний перезапуск процесу починає нову сесію перевірки оновлень.
...
- показує одну доступну теку журналу навігації компактним блоком «назва/шлях», а другу розміщує поруч;
...
Експорт зчитує доступну діагностику без перемикання режимів автомобіля чи відновлення дозволів. Також збирає потрібні для діагностики APK штатної навігації та приборки (включно зі split APK), бібліотеки та їхні залежності, файли framework і ресурси приборки. Уже авторизоване ADB надає ширший доступ; без нього залишаються доступні для читання локальні файли та базовий звіт. Відсутні або недоступні файли зазначаються в результаті.

Вікно експорту показує поточний етап і файл, а після пошуку — фактичну кількість файлів та скопійований обсяг. Збирання може тривати довго, а архів — перевищувати 1 ГБ. Його можна скасувати або згорнути й відкрити BYD HUD, поки той самий процес продовжує роботу. Кінцевий розмір архіву з’являється лише після перевірки ZIP. Ліміт надсилання розробнику — 20 MiB; більші архіви залишаються доступними через `Інший застосунок`, без поділу чи обрізання.
...
З авторизованим ADB запуск Logcat записує всі системні буфери та фіксує початковий стан швидкодії, а зупинка додає підсумкові показники кадрів і системи. Без ADB та сама кнопка зі змінним станом записує доступні застосунку буфери й діагностику. Пресетів фіксованої тривалості немає. Експорт конфігурації не запитує доступ ADB, не відновлює дозволи та не вмикає телеметрію.

Кожен запис Logcat ведеться в один безперервний файл до зупинки, без поділу, ротації та фіксованого обмеження розміру запису. Початкова й підсумкова діагностика залишаються окремими файлами в тій самій теці запису. Тривалі записи займають більше місця; звичайні налаштування зберігання журналів продовжують діяти.
