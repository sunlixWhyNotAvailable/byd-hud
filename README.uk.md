...
Застосунок також керує показом навігатора на панелі приладів, веде діагностичні логи за днями та може локально підготувати сумісний пакет навігатора для роботи через структурований прямий канал.
...
BYD HUD не є заміною Android Auto та не замінює і не переробляє інтерфейс навігатора на центральному дисплеї.
...
| Waze | Структурований маневр, відстань, вулиця, смуги та попередження сесії | Захоплення екрана та візуальний аналіз після увімкнення `Канал захоплення екрану (сумісність)` | Legacy-канал вимкнений за замовчуванням, залежить від розмітки екрана та більше не підтримується розробником |

Прямий канал не замінює мапу окремим surface. Waze або Google Maps продовжує показувати маршрут на планшеті та керувати ним, а навігаційні дані у фоновому режимі надсилаються в BYD HUD.
...
| Прямий вивід і патчення Google Maps | `app.revanced.android.apps.maps` | Google Maps ReVanced `25.16.03.747108139` |
...
- Для Google Maps потрібні видані дозволи та підключені служби захоплення Accessibility/сповіщень. Для Waze додатково потрібен непідтримуваний перемикач `Канал захоплення екрану (сумісність)`, який за замовчуванням вимкнений.
...
### Попередження та показники маршруту
...
| `Показувати ETA/час/дистанцію всього маршруту` | Вимкнено | Коли вимкнено, використовує показники до наступної зупинки; коли увімкнено, запитує показники всього маршруту, якщо джерело їх розрізняє |
...
Прямий канал Google Maps зараз надає повний набір показників. Поточна пряма сесія Waze не надає показники поїздки, тому ці перемикачі не мають видимого ефекту для Waze, доки джерело даних не зміниться.
...
Офіційний пакет Google Maps `com.google.android.apps.maps`, пакети з OBB-даними та непідтримувані структури feature-split відхиляються. Поточні захищені профілі патчера: Waze `4.95.0.3` / `5.20.0.1` та Google Maps ReVanced `25.16.03.747108139`.
...
3. Перегляньте індикатори стану компонентів. Waze окремо показує прямий канал, стабільну сесію та попередження Waze; Google Maps показує прямий та аудіоканал.
...
Шар прямого каналу обов'язковий. Опційні шари, наприклад навігаційний аудіоканал Google Maps або стабільна сесія і підтримка попереджень Waze, можуть бути недоступні, не заважаючи роботі прямого каналу. Alert-hook Waze наразі обмежений структурно сумісною версією Waze 5.20.0.1.
...
Перед патченням створіть резервну копію важливих даних навігатора та увійдіть у свій обліковий запис. Патчер ніколи не завантажує APK навігатора з інтернету та не надсилає вибрані APK на сервер.
...
BYD HUD не надсилає автоматично у сервіс Sentry збої, ANR, сесії, breadcrumbs, traces, скріншоти, записи екрана або навігаційні логи. Повна політика наведена у файлі [PRIVACY.md](PRIVACY.md).
...
- Поточна пряма сесія Waze не надає час прибуття, залишок часу або залишок дистанції маршруту.
...
- MaxTitan поділився початковою ідеєю прямого каналу та довідковими вихідними матеріалами.
