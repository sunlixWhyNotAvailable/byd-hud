...
For Basic and Extended, `Stop`, leaving the tab, putting BYD HUD in the background or switching modes ends the test and clears its output. Any still-active eligible navigation resumes. Starting again retains the selected example. Test choices do not alter the settings used by Waze or Google Maps. Authorized ADB enables the additional vehicle interface required for stock lanes and some other fields on supported cars; unavailable outputs do not prevent the others from being tested. The TBT card uses its own maneuver and text layout, not a copy of the HUD lane or bitmap image.

### Shanghai test

`Shanghai test` has its own Start/Stop controls. With authorized local ADB, Start sets a simulated GPS position in Shanghai. During the initial 15 seconds, open the stock navigator and start guidance with HUD output. The saved route runs for 4 minutes 55 seconds including that initial wait. It continues when you change tabs or open the navigator, without changing the automatic-start preference. BYD HUD pauses its own HUD/TBT output during the test while continuing to receive navigation data.

Logcat is mandatory. An existing recording continues through and after the test; otherwise the test starts one and finishes it after GPS cleanup and a further 5 seconds of recording. HUD check mode selection and the Logcat button remain locked until completion. Available SOME/IP, ADAS, packet capture and state snapshots are saved with the logs; unavailable channels and losses are identified. A completed route does not confirm what appeared on the physical HUD.

`Reset mock location` removes the current GPS simulation, including one created by another app, and ends an active Shanghai test. Automatic recovery on opening/enabling BYD HUD only cleans up its own interrupted session. A failed cleanup stays visible for retry. Recovery after a process crash requires the next opening/enabling of the app; an immediate real position or map recentering is not guaranteed. Another app can start a new simulation after a manual reset.
