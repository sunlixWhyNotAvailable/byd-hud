...
`Share configuration` is a separate user-requested export. It can include device properties, installed packages and signers, running processes and services, network topology and routes, SOME/IP configuration, and navigation-related system configuration. IP, IPv6, MAC/BSSID, and peer addresses are replaced with stable aliases inside each archive while interface names, route structure, prefixes, ports, and process names remain available for diagnosis. Sensitive configuration values such as credentials, VIN, location, and account identifiers are redacted before inclusion. When local ADB is already authorized, the archive can also include a read-only catalog of static BYD SDK feature IDs. It never requests ADB access solely for this export and does not read or write live vehicle feature values.
