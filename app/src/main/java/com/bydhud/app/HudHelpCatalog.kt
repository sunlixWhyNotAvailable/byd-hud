...
internal object HudHelpCatalog {
    val topics: List<HudHelpTopic> = listOf(
        ...
        topic(HudHelpTopicId.WazeAlertField, "Поле виводу попередження Waze", "Waze alert output field", frame("Маневру", "Maneuver", "Попередження у полі маневру; маршрутна дистанція та вулиця тимчасово замінюються, смуги залишаються.", "Warnings use the maneuver field; route distance and street are temporarily replaced while lanes remain.", R.drawable.hud_help_warning_maneuver), frame("Експериментальне", "Experimental", "Вибір зберігається для наступного патчу; зараз попередження залишаються у полі маневру.", "The choice is saved for the next patch; warnings currently remain in the maneuver field.", R.drawable.hud_help_warning_separate))
    )
    ...
    fun etaImage(street: Boolean, mask: Int): Int = if (street) {
